...
namespace ABMath.ModelFramework.Transforms
{
    ...
    public class SamplingTransformation : TimeSeriesTransformation, IExtraFunctionality
    {
        ...
        [CategoryAttribute("Parameter"), DescriptionAttribute("Time to begin sampling")]
        public DateTime StartTime { get; set; }
        [CategoryAttribute("Parameter"), DescriptionAttribute("Time to stop sampling")]
        public DateTime EndTime { get; set; }
        [CategoryAttribute("Parameter"), DescriptionAttribute("List of lags (after each sample point) to get data")]
        public List<TimeSpan> SamplingBaseOffsets { get; set; }
        [CategoryAttribute("Parameter"), DescriptionAttribute("Interval for sampling, set to 0 to take all available points")]
        public TimeSpan SamplingInterval { get; set; }
        [CategoryAttribute("Parameter"), DescriptionAttribute("Set to true if sampler should skip weekends")]
        public bool SkipWeekends { get; set; }
        [CategoryAttribute("Parameter"), DescriptionAttribute("List of days to be skipped")]
        ...
        public bool AuxiliaryFunction(int idx, out object output)
        {
            ...
            switch (idx)
            {
                case 0:
                    if (tsOutput != null)
                    {
                        // copy to clipboard but using sample offsets to define an array
                        int m = SamplingBaseOffsets.Count;
                        var sb = new StringBuilder(16384);
                        for (int t = 0; t < tsOutput.Count; ++t)
                        {
                            int index = t % m;
                            sb.AppendFormat("{0:0.000000}", tsOutput[t]);
                            if (index != (m - 1))
                                sb.Append("\t");
                            else
                    ...
                    if (mvtsOutput != null)
                    {
                        ...
                        int m = SamplingBaseOffsets.Count;
                        var sb = new StringBuilder(16384);
                        for (int t = 0; t < mvtsOutput.Count; ++t)
                        {
                            int index = t % m;
                            for (int i = 0; i < mvtsOutput.Dimension; ++i)
                                sb.AppendFormat("{1}{0:0.000000}", mvtsOutput[t][i], (i != 0 ? "\t" : ""));
                            if (index != (m - 1))
                                sb.Append("\t");
                            else
                                sb.Append(Environment.NewLine);
                ...
                case 1:
                    ...
                    var mvts = GetInput(0) as MVTimeSeries;
                    var ts = GetInput(0) as TimeSeries;
                    DateTime beginning = StartTime, ending = EndTime;
                    if (ts != null)
                    {
                        beginning = ts.TimeStamp(0);
                        ending = ts.GetLastTime();
                    }
                    if (mvts != null)
                    {
                        beginning = mvts.TimeStamp(0);
                        ending = mvts.GetLastTime();
                    }
                    StartTime = beginning;
                    EndTime = ending;
                    return true;
