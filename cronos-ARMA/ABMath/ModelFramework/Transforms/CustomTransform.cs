...
namespace ABMath.ModelFramework.Transforms
{
    ...
    public class CustomTransform : TimeSeriesTransformation
    {
        ...
        [CategoryAttribute("Parameter"), Description("Mathematical expression as a function of x,y and z.")]
        public string Expression { get; set; }
        [CategoryAttribute("Parameter"), Description("Number of inputs (1 to 3).")]
        public int NumInputVariables { get; set; }
        ...
        private void CompileStuff()
        {
            ...
            var forLoop = new CodeIterationStatement(
                ...
                new CodeExpressionStatement(new CodeSnippetExpression("x[testInt] = MyExp(x[testInt],y[testInt],z[testInt])")));
            ...
            CompilerResults dom = pro.CompileAssemblyFromDom(new CompilerParameters(), unit);
            if (dom.Errors.Count > 0)
                return;

            Assembly assembly = dom.CompiledAssembly;
            Type type = assembly.GetType("CustomNamespace.CustomClass");
        ...
        public override void Recompute()
        {
            ...
            if (methodInfo == null)
            {
                MessageBox.Show("Unable to parse expression.", "Error");
                return;
            }
            ...
            {
                double[] tempxArray = new double[tsList[0].Count];
                double[] tempyArray = new double[tsList[0].Count];
                double[] tempzArray = new double[tsList[0].Count];

                for (int t = 0; t < tsList[0].Count; ++t)
                {
