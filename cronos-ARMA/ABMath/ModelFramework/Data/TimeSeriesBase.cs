...
namespace ABMath.ModelFramework.Data
{
    ...
    public class TimeSeriesBase<T>
    {
        ...
        {
            exact = false;
            int sidx = times.BinarySearch(time);
            if (sidx < 0)
                sidx = ~sidx-1;
            else
                exact = true;
        ...
        public void InsertInMiddle(DateTime time, T value, bool forceOverwrite)
        {
            ...
            int i0 = ~sidx - 1; // the index of the time before the searched time
        ...
        public TimeSpan GetCommonSamplingInterval()
        {
            ...
            for (int t=times.Count-1 ; t>0 && t>times.Count-100 ; --t)
            {
        ...
        /// <param name="forceOverwrite">if timestamp already exists, determines whether to throw an exception or overwrite existing value</param>
        public void Add(DateTime time, T value, bool forceOverwrite)
        {
            // do a little check
            bool comesAtEnd = true;
            if (times.Count > 0)
            {
